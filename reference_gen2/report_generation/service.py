...
def _render_header(
    ...
    source_label = _source_mode_label(payload.get("source_mode", "unknown"))
    return f"""
<header class="report-header">
  <div class="title-row">
    <div>
      <h1>{_e(config.title)}</h1>
      <p class="meta">ID: {_e(cycle_id)} · Stijl: {_e(style_label)} · Bron: {_e(source_label)}</p>
    </div>
    {status_chip}
  </div>
  <div class="header-controls" aria-label="Zoeken in rapport">
    <label class="search-label">
      <span>Zoeken</span>
      <input id="reportSearch" type="search" placeholder="zoek naar specifieke bronnen">
    </label>
    <div class="view-controls" aria-label="Rapportweergave">
      <button class="view-toggle" id="saveReportButton" type="button">Rapport opslaan</button>
      <button class="view-toggle active" id="fullViewToggle" type="button" data-view="full">Volledig rapport</button>
      <button class="view-toggle" id="basicViewToggle" type="button" data-view="basic">Basis / print</button>
    </div>
  </div>
  <div class="stats-grid" id="reportFilters" aria-label="Rapportfilters">
    <button class="stat active" type="button" data-filter="all"><span>{total}</span><small>Bronnen</small></button>
    <button class="stat good" type="button" data-filter="verified"><span>{counts["verified"]}</span><small>Geverifieerd</small></button>
    <button class="stat warn" type="button" data-filter="needs_review"><span>{counts["needs_review"]}</span><small>Controleren</small></button>
    <button class="stat bad" type="button" data-filter="suspicious"><span>{counts["suspicious"]}</span><small>Verdacht</small></button>
  </div>
...
def _render_progress(counts: dict[str, int], total: int) -> str:
    ...
    return '<div class="progress" aria-label="Verdeling van bronstatussen">' + "".join(segments) + "</div>"
...
def _render_type_filters(type_counts: dict[str, int], total: int) -> str:
    if total <= 0:
        return ""
    buttons = [
        '<button class="type-chip active" type="button" data-type-filter="all">Alle typen</button>'
    ]
    ...
    return (
        ...
        '<p class="filter-label">Brontype</p>'
        f'<div class="type-filter-grid" id="typeFilters" aria-label="Brontypefilters">{"".join(buttons)}</div>'
        "</div>"
    )
...
<section class="summary-panel">
  <h2>Documentsamenvatting</h2>
  <div class="summary-grid">
    <div><span>Uploadtype</span><b>{_e(_source_mode_label(phase1.get("upload_kind", "unknown")))}</b></div>
    <div><span>Gevonden bronnen</span><b>{_e(phase2.get("reference_count", 0))}</b></div>
    <div><span>Totale generatietijd</span><b>{_format_seconds(total_ms)}</b></div>
  </div>
</section>
""".strip()
...
def _render_reference_list(references: list[dict[str, Any]]) -> str:
    if not references:
        return '<section class="empty-state"><h2>Geen bronnen</h2><p>Er waren geen bronsamenvattingen beschikbaar voor dit rapport.</p></section>'
    cards = "\n".join(_render_reference_card(row) for row in references)
    return f'<section class="reference-list" id="referenceList">{cards}<div class="empty-state hidden" id="emptyFiltered"><h2>Geen resultaten</h2><p>Geen bronnen komen overeen met het huidige filter.</p></div></section>'
...
<article class="reference-card status-border-{_status_class(filter_status)}" data-status="{_e(filter_status)}" data-source-type="{_e(source_type)}" data-hay="{_e(haystack)}">
  <div class="card-main">
    <div>
      <p class="card-kicker">Bron {row["index"]}</p>
  ...
  <details>
    <summary>Verificatiestappen</summary>
...
def _render_next_steps(row: dict[str, Any], filter_status: str) -> str:
    if filter_status not in {"needs_review", "suspicious"}:
        return ""
    guidance_html = _next_step_guidance_html(row)
    actions = _source_check_actions(row, filter_status)
    action_html = ""
    if actions:
        action_html = '<div class="source-check-actions">' + "".join(
            f'<a class="source-check-link" href="{_e(action.href)}" target="_blank" rel="noopener noreferrer">{_e(action.label)}</a>'
            for action in actions
        ) + "</div>"
    return (
        '<section class="next-steps">'
        "<h3>Controleer deze bron</h3>"
        f"<p>{guidance_html}</p>"
        f"{action_html}"
        "</section>"
    )
...
def _next_step_guidance_html(row: dict[str, Any]) -> str:
    if _has_doi_not_found(row):
        return (
            "De DOI uit de bron is niet teruggevonden in de database. Voor extra informatie kijk hieronder bij "
            "<i>Bron vs gevonden bron</i> om te zien hoe de resultaten verschillen. Controleer deze bron handmatig "
            "in bijvoorbeeld Google Scholar of de DOI in doi.org."
        )
    if _has_database_match(row):
        return (
            "Een mogelijke match gevonden in de database. Deze komt niet helemaal overeen met de aangeleverde bron. "
            "Voor extra informatie kijk hieronder bij <i>Bron vs gevonden bron</i> om te zien hoe de resultaten "
            "verschillen. Controleer deze bron handmatig in bijvoorbeeld Google Scholar. Vergelijk vooral titel, "
            "tijdschrift, jaar, pagina's en DOI met de gevonden bron."
        )
    return (
        "Geen overeenkomst gevonden in de database. Controleer titel, auteurs, jaar en brongegevens handmatig. "
        "Vul ontbrekende gegevens aan voordat je de bron opnieuw beoordeelt."
    )
...
def _match_text_block(row: dict[str, Any]) -> str:
    ...
    if not accepted and not runner and not accepted_render and not runner_render:
        return ""
    accepted_html = _match_citation_item("Beste gevonden bron", accepted_render, accepted)
    runner_html = _match_citation_item("Tweede kandidaat", runner_render, runner)
    return f"""
...
def _field_comparison_block(comparisons: list[dict[str, Any]]) -> str:
    ...
    rows = "\n".join(
        f"""
<tr>
  <td>{_e(_comparison_field_label(item.get("label", "")))}</td>
  ...
  <td><span class="mini-status status-{_comparison_status_class(item.get("status"))}">{_e(_comparison_status_label(item.get("status", "")))}</span></td>
</tr>
""".strip()
...
<details class="field-comparison">
  <summary>Bron vs gevonden bron</summary>
  <div class="table-wrap"><table><thead><tr><th>Veld</th><th>Bron</th><th>Gevonden bron</th><th>Score</th><th>Status</th></tr></thead><tbody>{rows}</tbody></table></div>
</details>
""".strip()
...
def _source_only_comparisons(row: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if not isinstance(fields, dict):
        return []
    container_label = "Tijdschrift"
    source_type = _source_type_key(row.get("ctype"))
    if source_type == "book":
        container_label = "Uitgever"
    elif source_type == "book_chapter":
        container_label = "Boek"
    ...
    add("Titel", fields.get("Title"))
    add("Auteurs", fields.get("Authors") or fields.get("Editors"))
    add("Jaar", fields.get("Year") or fields.get("Date"))
    add(
        container_label,
...
def _source_check_actions(row: dict[str, Any], filter_status: str) -> list[_SourceCheckAction]:
    if filter_status not in {"needs_review", "suspicious"}:
        return []
    source_type = _source_type_key(row.get("ctype"))
    fields = row.get("parsed_fields") if isinstance(row.get("parsed_fields"), dict) else {}
    title = _field_text(fields, "Title")
    author = _first_author_token(
        _field_text(fields, "Authors") or _field_text(fields, "Editors")
    )
    container = _field_text(fields, "Container")

    doi_actions = _doi_org_actions(row) if _has_doi_not_found(row) else []

    if source_type == "webpage":
        url = _field_text(fields, "URL") or _field_text(fields, "Access URL") or _first_url(row.get("display_reference"))
        website_actions = [_SourceCheckAction("Website openen", url)] if url else []
        return website_actions + doi_actions
    if source_type == "book":
        return _openlibrary_actions(_title_without_subtitle(title), author) + doi_actions
    if source_type == "book_chapter":
        book_title = _openlibrary_book_title_query(container or title)
        editor = _first_chapter_editor_token(fields, row)
        return _openlibrary_actions(book_title, editor) + doi_actions
    if source_type == "journal_article":
        return _scholar_actions(title, author, include_author=_word_count(title) <= 3) + doi_actions
    if source_type == "conference_paper" and filter_status == "needs_review":
        return _scholar_actions(title, author, include_author=_word_count(title) <= 3) + doi_actions
    return doi_actions


def _openlibrary_actions(title: str, author: str) -> list[_SourceCheckAction]:
    if not title:
        return []
    params = [f"title={quote_plus(title)}"]
    if author:
        params.append(f"author={quote_plus(author)}")
    return [_SourceCheckAction("Zoek in OpenLibrary", "https://openlibrary.org/search?" + "&".join(params))]


def _scholar_actions(title: str, author: str, *, include_author: bool) -> list[_SourceCheckAction]:
    if not title:
        return []
    query = f"{title} {author}".strip() if include_author and author else title
    href = (
        "https://scholar.google.com/scholar?hl=nl&as_sdt=0%2C5&q="
        f"{quote_plus(query)}&btnG="
    )
    return [_SourceCheckAction("Zoek in Google Scholar", href)]


def _doi_org_actions(row: dict[str, Any]) -> list[_SourceCheckAction]:
    doi = _source_doi(row)
    if not doi:
        return []
    return [_SourceCheckAction("Open DOI in doi.org", f"https://doi.org/{doi}")]


def _source_doi(row: dict[str, Any]) -> str:
    fields = row.get("parsed_fields") if isinstance(row.get("parsed_fields"), dict) else {}
    for value in (fields.get("DOI"), row.get("display_reference")):
        doi = _extract_first_doi(value)
        if doi:
            return doi
    return ""


def _extract_first_doi(value: Any) -> str:
    text = str(value or "")
    match = _DOI_TEXT_RE.search(text)
    if not match:
        return ""
    doi_value = (
    ...
    _stripped_text, stripped_doi, _trailing = _strip_doi_trailing_punctuation(
        match.group(0),
        doi_value,
    )
    return stripped_doi
...
def _has_database_match(row: dict[str, Any]) -> bool:
    return bool(
        row.get("accepted_match_display")
        or row.get("accepted_match_render")
        or row.get("accepted_record_id")
        or row.get("best_candidate_display")
        or row.get("candidate_count")
    )


def _has_doi_not_found(row: dict[str, Any]) -> bool:
    return "EXTRACTED_DOI_NOT_FOUND_IN_DB" in _evidence_codes(row)


def _evidence_codes(row: dict[str, Any]) -> set[str]:
    codes: set[str] = set()
    for check in row.get("evidence_checks") or []:
...
def _title_without_subtitle(title: str) -> str:
    text = title.strip()
    if not text:
        return ""
...
def _openlibrary_book_title_query(title: str) -> str:
    text = _title_without_subtitle(title)
    if not text:
        return ""
    text = text.replace("\u00ad", "")
    text = re.sub(r"(?<=[A-Za-z])-\s*(?=[a-z]{1,4}\b)", "", text)
    return " ".join(text.split())


def _first_chapter_editor_token(fields: dict[str, Any], row: dict[str, Any]) -> str:
    editor = _first_author_token(_field_text(fields, "Editors"))
    if editor:
        return editor
    editor = _first_inline_editor_token(row.get("display_reference"))
    if editor:
        return editor
    return _first_author_token(_field_text(fields, "Authors"))


def _first_inline_editor_token(value: Any) -> str:
    text = str(value or "")
    match = re.search(
        r"\bin\s+(?P<names>.+?)\s*\(\s*(?:eds?\.?|editors?|red\.?|reds\.?)\s*\)",
        text,
        flags=re.IGNORECASE,
    )
    if not match:
    ...
    first = re.split(r"\s+(?:and|en)\s+|\s*&\s*|;", names, maxsplit=1)[0].strip()
    return _first_author_token(first)


def _first_author_token(value: str) -> str:
...
def _first_url(value: Any) -> str:
    match = _URL_TEXT_RE.search(str(value or ""))
    if not match:
        return ""
    url, href, _trailing = _strip_url_trailing_punctuation(match.group(0))
    return href or url
...
def _evidence_table(checks: list[dict[str, Any]]) -> str:
    if not checks:
        return '<p class="empty-inline">Er zijn geen bewijscontroles beschikbaar.</p>'
    rows = "\n".join(
        f"""
<tr>
  <td>{_e(check.get("label") or check.get("code", ""))}</td>
  <td><span class="mini-status status-{_status_class(check.get("status", ""))}">{_e(_status_text(check.get("status", "")))}</span></td>
  <td>{_e(check.get("summary", ""))}</td>
</tr>
""".strip()
        for check in checks
    )
    return f'<div class="table-wrap"><table><thead><tr><th>Controle</th><th>Status</th><th>Samenvatting</th></tr></thead><tbody>{rows}</tbody></table></div>'
...
def _report_style_label(payload: dict[str, Any]) -> str:
    ...
    requested = str(payload.get("requested_style_hint") or "unknown")
    resolved_label = _display_style_label(resolved)
    if requested == "unknown" and resolved != "unknown":
        return f"{resolved_label} (auto)"
...
def _source_type_label(value: Any) -> str:
    ...
    labels = {
        "book": "Boek",
        "book_chapter": "Boekhoofdstuk",
        "conference_paper": "Conferentiebijdrage",
...
def _label(value: Any) -> str:
    return _e(_status_text(value))


def _status_text(value: Any) -> str:
    normalized = str(value or "unknown").strip().lower().replace("-", "_")
    labels = {
        "candidate_only": "Kandidaat gevonden",
        "error": "Fout",
        "failed": "Mislukt",
        "matched": "Match",
        "matched_provisional": "Voorlopige match",
        "needs_review": "Controleren",
        "no_match": "Geen match",
        "not_applicable": "Niet van toepassing",
        "not_run": "Niet uitgevoerd",
        "ok": "Ok",
        "pass": "Geslaagd",
...
def _comparison_field_label(value: Any) -> str:
    normalized = str(value or "").strip().lower()
    labels = {
        "authors": "Auteurs",
        "author": "Auteur",
        "book": "Boek",
        "container": "Tijdschrift/boek",
        "doi": "DOI",
        "journal": "Tijdschrift",
        "metadata": "Metadata",
        "pages": "Pagina's",
...
def _comparison_status_label(value: Any) -> str:
    normalized = str(value or "unknown").strip().lower().replace("-", "_")
    labels = {
        "found": "Gevonden",
        "match": "Match",
    ...
    return labels.get(normalized, _status_text(normalized))
...
.source-check-actions { display: flex; flex-wrap: wrap; gap: 8px; margin-top: 10px; }
.source-check-link {
  display: inline-flex;
  align-items: center;
  min-height: 34px;
  padding: 7px 10px;
  border: 1px solid #d89c44;
  border-radius: 8px;
  background: #fff;
  color: #70410d;
  font-weight: 750;
...
.source-check-link:hover { background: #fff9ef; border-color: #b9781e; }
...
(() => {
  ...
  function reportFilename() {
    const title = document.title || "referentierapport";
    const safe = title
      .toLowerCase()
      .replace(/[^a-z0-9]+/g, "-")
      .replace(/^-+|-+$/g, "")
    ...
    return `${safe || "referentierapport"}-${date}.html`;
  }
